...
public class Map implements Defines {
  ...
  public Map(int diff, Replay replay) {
    mapX = MAP_BASE_SIZE_X;
    mapY = MAP_BASE_SIZE_Y;
    difficultyModifer = diff;
    playerX = startX = MAP_PLAYER_START_POS_X;
    ...
    for (int i = 0; i < mapY; i++) {
      for (int j = 0; j < mapX; j++) {
       // fields[i][j].visible = true;//for test only
        try {
          switch (replay.fileInput.read()) {
            case 'X': {// disabled field
              fields[i][j].enabled = false;
              break;
            }
            case '!': {// exit field
              fields[i][j].enabled = true;
              fields[i][j].exit = true;
              break;
            }
            case 'M': {// monster field
              fields[i][j].enabled = true;
              fields[i][j].status = MAP_EVENT_BATTLE;
              break;
            }
            case 'T': {// trap field
              fields[i][j].enabled = true;
              fields[i][j].status = MAP_EVENT_TRAP;
              break;
            }
            case 'S': {// treasure field
              fields[i][j].enabled = true;
              fields[i][j].status = MAP_EVENT_TREASURE;
              break;
            }

            case 'E': {// event field
              fields[i][j].enabled = true;
              fields[i][j].status = MAP_EVENT_EVENT;
              break;
            }
            case ' ': {// empty field
              fields[i][j].enabled = true;
